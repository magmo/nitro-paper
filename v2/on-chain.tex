...
In this paper, we assume that ledger channels are fully funded when an exit is triggered, ie. $$\holdings(X) = \sum_{e\in \statusOf(X).\outcome} e.\amount.$$ There are multiple ways of implementing deposits, and the main result \ref{main-result}\todo{missing ref} does not depend on this choice.
...
Guarantees are triggered via the \textbf{claim}\todo{If we call this \em{reclaim} instead of claim, and simply modify outcomes, we'll end up with a cleaner statement.} operation \claim{A}{i}\todo{decide where to fetch $x$ from}, which follows the following specification:
\begin{enumerate}
  \item Reverts if the channel $A$ is not finalized\todo{address}.
