...
\begin{abstract}
  ...
  In this paper we present constructions that allow state channels to be fund one another, reducing the number of on-chain deposits required.
...
By allowing participants to execute multiple transitions off-chain, the state channel removes load from the blockchain, allowing it to support the same level of activity with fewer transactions.
...
Each time a state channel is opened at least one party needs to perform an on-chain transaction to transfer assets into the state channel, and each time it is closed at least one participant must perform an on-chain transaction to claim their share.
...
they already have an existing ledger channel, $\rchi_L$, open between them.
...
Updates to the chess channel are independent from updates to the ledger channel $\rchi_L$ and to
...
By using a virtual channel, Alice and Bob may open their chess game, using their existing ledger channels with Carol, without the need for any on-chain transactions.
...
\begin{itemize}
  ...
  \item \textbf{Ledger channels}: on the order of one on-chain deposit per group of interacting participants, regardless of how many channels they participate in together.
  \item \textbf{Virtual channels}: on the order of one on-chain deposit per user, regardless on how many channels they participate in.
\end{itemize}
...
consensus game transition to move between two network states with the same value to a
...
If I have two network states, $\Sigma$ and $\Sigma'$, which only differ in the state of
...
applying a consensus game transition gives a sequence of network states 
...
Therefore, at all points, the value of the network to $p$ is $x$.
