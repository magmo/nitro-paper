...
In this section we will present a model for state channel networks, which will serve as a foundation
for the protocols introduced later in the paper.
It will also form the basis for the section \ref{sec:reasoning}, where we present the logic
to reason about state channels.
...
- the purpose of the state channel is to get to an 'outcome'
- in this section, we will use the simplest possible outcome - allocations

- how do we create an outcome and get it on-chain
...
- you can only withdraw from a participant address
- you can only get an outcome for a channel address
...
The model is opinionated, in that it makes certain choices about the working details of state channel
networks that will not be true in general.
