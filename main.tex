...
In this paper, we allow channels to be opened and closed off-chain. We enable the construction of efficient state channel networks.\todo{TODO}
...
- constrained
\todo{Connext - have removed the time limit with a trade-off of trusting the hub - check this}
...
\todo{
  How about jus tusing $\alpha = \{(\chi, x)\}$ to denote that, in allocation $\alpha$, $\chi$ holds $x$?
  IE, $\alpha$ is a function from the set of channels to $\mathbb{R}$, and we could say
  $\alpha: \chi \mapsto x$, or $\alpha(\chi) = x$, like normal.

  We could use $\overline{\alpha}$ to indicate that $\alpha$ 
}
...
\end{align*} \todo{
  ...
  I feel like I should be able to guess what it means without seeing the definition.
}
...
where \texttt{signature} is $A$'s\todo{\texttt{fromAddr}'s signature? or, specify that \texttt{fromAddr} is $A$'s address?} signature of the other parameters
...
to be the largest $x$ such that $A$ has an \textit{unbeatable strategy}\todo{I wonder if it makes sense to use terminology from game theory. IE, an unbeatable strategy to withdraw $x$ is just a strategy that guarantees an eventual withdrawal of $x$ (in bounded time)} to withdraw $x$
...
In terms of applying operations on-chain, we need to \todo{finish sentence}
...
\todo{Ignore gas + censorship, allow front-running}
...
\begin{enumerate}
  \item No participants apart from $p$ have a finalizable outcome.
        ...
        \todo{Swap statements -- who is $p$ in the first sentence?}
...
except the last case, which usually means something has gone wrong. \todo{``that the first two possibilities naturally...''}
...
Another is to transition to transition to\todo{repetition} the\todo{replace `the' with `a'} conclude state, to allow a conclusion proof
...
$\enf{\beta_\rchi(\omega)}_{p_1,\dots, p_n}$.
We will sometimes write this state using the shortened notation $\enf{\beta_\rchi(\omega)}_P$.\todo{This is confusing, because above, $P$ is used to denote a subset of participants}
...
a consensus game chanel\todo{channel}, $\rchi$, with participants $P$ and universally enforceable outcomes
...
\end{center}
\todo{The channel $\rchi_L$ should probably be referenced somewhere in this graphic. (Likewise below)}
...
\end{align*}\todo{Perhaps start with $S_3$, matching the above states}
where $x = a + a' + b + b'$.
...
\begin{example}
  ...
  \end{multline*}\todo{This should probably be expanded over multiple lines, showing the effects of the application of $G_{A, B, C}(2)$, followed by the effect of $G_{A', B, D}$}
...
\todo{The claim {\em operator} is called $G$, and the claim {\em operation} is called $T$. This is confusing.}
\todo{You should specify that $y \geq x$}
